...
pub struct OscApp {
    ...
    local_cache: Vec<OscElement>,
}

impl eframe::App for OscApp {
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        if let Ok(guard) = self.parameters.try_lock() {
            if !guard.is_empty() && self.local_cache.len() != guard.len() {
                self.local_cache = guard.clone();
            } else if self.local_cache.len() == guard.len() {
                for (local, shared) in self.local_cache.iter_mut().zip(guard.iter()) {
                    local.value = shared.value.clone();
                }
            }
        }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                egui::Grid::new("interactive_osc_grid")
                    ...
                    .striped(true)
                    .show(ui, |ui| {
                        for element in self.local_cache.iter_mut() {
